<form action="{{ route('timer-cards.start', $id) }}" method="POST" class="bg-base-100 shadow-md rounded-lg p-4 mt-4 max-w-sm mx-auto">
    @csrf
    <div class="flex justify-between items-center">
        <h2 class="text-lg font-semibold text-base-500">{{ $cardName }}</h2>
        <div class="flex space-x-2">
            <span class="cursor-pointer text-base hover:text-warning" onclick="openEditModal('{{ $id }}', '{{ $cardName }}', '{{ $therapistName }}', '{{ $time }}')">
                <!-- Ikon Edit -->
