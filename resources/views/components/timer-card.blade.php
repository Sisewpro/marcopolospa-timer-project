<form action="{{ route('timer-cards.start', $id) }}" method="POST" onsubmit="event.preventDefault();"
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-2">
            <span id="editModal_{{ $id }}" class="cursor-pointer text-base hover:text-primary"
    ...
    <div class="text-center mt-1">
        ...
        <div class="-mt-2">
            <progress id="progressBar_{{ $id }}" class="progress progress-warning w-3/5 h-2" value="100"
                max="100"></progress>
        </div>
    </div>
...
function initializeTimerCard(cardId, initialTime, status, startTime) {
    ...
    function startTimer() {
        ...
        fetch(`/timer-cards/${cardId}/start`, {
            ...
            body: JSON.stringify({
                customer: "{{ $customer }}", // Ambil value dari backend
                time: totalSeconds,
                status: 'Running'
            })
        });
